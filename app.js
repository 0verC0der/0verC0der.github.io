...
async function onLangRefresh(newLang) {
	...
	if (lastREQ.data) {
		try {
			const rev = await fetch(GEO_REV(lastREQ.lat, lastREQ.lon, lang)).then(r =>
				r.json()
			)
			lastREQ.label = rev
				? [rev.locality, rev.countryName].filter(Boolean).join(', ')
				: lastREQ.label
		} catch (e) {
			console.error(e)
...
const onInput = waiter(async event => {
	...
	if (query.length < 2) {
		listboxObj.closeListBox()
		return
	}
	const cacheKey = `${query.toLowerCase()}| ${lang} `
	if (cache.has(cacheKey)) {
		listboxObj.render(cache.get(cacheKey), query, I18n[lang])
		return
	}
	abortCtrl?.abort()
	...
	try {
		const response = await fetch(GEO_API_URL(query, lang), {
...
async function fetchAndRenderWeather(lat, lon, label) {
	try {
		...
		const time = await fetch(TIME_BY_TIMEZONE(data.timezone)).then(r =>
			r.json()
		)

		console.log('Weather data:', data)
		data.current_timezone_time = time.datetime
			.replace(/\.\d+(?=([+-]\d{2}:?\d{2}|Z)?$)/, '')
			.replace(/([+-]\d{2}:?\d{2}|Z)$/, '')
		...
		for (let i = 0; i < daily.time.length; i++) {
			...
			day.innerHTML = `
                        <div class="muted"> ${ftmDate(
                            ...
                            <div> <img src="svg/${
															ICON_BY_WMO[dayCode] || ICON_BY_WMO[0]
														}" alt="${weatherText(
				dayCode,
				lang
...
function setClockTextContent() {
	...
	const timeSTR = new Intl.DateTimeFormat(lang || 'en', {
		hour: '2-digit',
		minute: '2-digit',
		second: '2-digit',
	}).format(dt)
